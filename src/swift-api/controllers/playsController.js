...
async function getMostFeaturedArtist(req, res) {
  const year = parseInt(req.params.year);
  ...
  try {
    const mostFeaturedArtist = await playService.getMostFeaturedArtist(year);
    if (mostFeaturedArtist) {
      return res.status(200).json(mostFeaturedArtist);
    } else {
      return res
        .status(404)
        .json({ message: "No artist data found for the given year" });
    }
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
